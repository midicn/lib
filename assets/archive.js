...
const SOURCES = [
  {
    ...
    method:{zh:'下载官方 metadata.json 与 MIDI 集；作品名称由我们以 IMSLP 官方作品目录反查补全（按各作曲家编号体系严格匹配）。',
            en:'Official metadata.json and MIDI set; work titles recovered via IMSLP catalogue matching.'},
  ...
  },
  {
    ...
    method:{zh:'通过官方 API 获取曲调元数据与 ABC 记谱，转译为 MIDI。',
            en:'Fetched via the official API; ABC notation rendered to MIDI.'},
  ...
  },
  {
    ...
    method:{zh:'采集官方 MIDI 与曲目页元数据；年代由 MIDI 内嵌信息与曲调页交叉校验。',
            en:'Official MIDI plus tune-page metadata; years cross-checked against embedded MIDI info.'},
  ...
  },
  {
    ...
    method:{zh:'由 ESAC 数据集中的中国卷 ABC 记谱转译，region 沿用采录地（精确到县）。',
            en:'Rendered from the China volumes of the ESAC ABC corpus; region kept at county level.'},
  ...
  },
  {
    ...
    method:{zh:'解析 ABC 记谱；国家由曲目自带的 O: 地理层级字段推导（中国曲目精确到省）。',
            en:'ABC records parsed; country derived from per-tune O: geographic hierarchy.'},
  ...
  },
  {
    ...
    method:{zh:'下载官方 MIDI 与 composers_manually_checked.csv；国籍与生卒年取自该官方表。',
            en:'Official MIDI plus composers_manually_checked.csv for nationality and dates.'},
  ...
  },
  {
    ...
    method:{zh:'文件名解析编号（BWV/K./Op.）与作曲家线索；乐器由 MIDI program change 事件挖掘。',
            en:'Catalogue numbers parsed from file names; instruments mined from MIDI program changes.'},
  ...
  },
  {
    ...
    method:{zh:'解析 ABC 头字段（R: 曲式 / O: 地域）后转译 MIDI。',
            en:'ABC header fields (R: form, O: region) parsed, then rendered to MIDI.'},
  ...
  },
  {
    ...
    method:{zh:'由语料中的 MusicXML 转译 MIDI；时期参照作曲家生卒推断，标题规范化（BWV 等）。',
            en:'MusicXML from the corpus rendered to MIDI; periods inferred from composer dates.'},
  ...
  },
  {
    ...
    method:{zh:'读取官方 .ly 源文件头部（标题 / 作曲家 / 编号 / 乐器 / 年代 / 许可），据此校正元数据。',
            en:'Official .ly headers parsed for title, composer, opus, instrument, date and licence.'},
  ...
  },
  {
    ...
    method:{zh:'直接解析 ABC 头字段（T: 标题 / C: 作曲 / R: 曲式）后转译。',
            en:'ABC headers (T:, C:, R:) parsed directly, then rendered.'},
  ...
  },
  {
    ...
    method:{zh:'由 MuseScore 工程文件转译 MIDI；作曲家人名与生卒用于时期推断。',
            en:'MuseScore sources rendered to MIDI; composer dates used for period inference.'},
  ...
  },
  {
    ...
    method:{zh:'取官方 MIDI 与元数据（作曲家 / 作品 / 年份）。',
            en:'Official MIDI and metadata (composer, title, year).'},
  ...
  },
  {
    ...
    method:{zh:'取官方 MIDI 与 info.csv（速度 / 拍号 / 风格）。',
            en:'Official MIDI plus info.csv (tempo, meter, style).'},
  ...
  },
  {
    ...
    method:{zh:'取官方 MIDI 与情绪标注表。',
            en:'Official MIDI and emotion annotation table.'},
  ...
  },
  {
    ...
    method:{zh:'解析 ABC 曲式字段（R:）后转译 MIDI。',
            en:'ABC form field (R:) parsed, then rendered.'},
  ...
  },
  {
    ...
    method:{zh:'由 MusicXML 乐谱转译 MIDI。',
            en:'MusicXML sources rendered to MIDI.'},
  ...
  },
  {
    ...
    method:{zh:'按素材页逐曲记录许可后转译 MIDI。',
            en:'Per-asset licence recorded, then rendered to MIDI.'},
  ...
  },
  {
    ...
    method:{zh:'取官方 musicnet_metadata.csv 与参考 MIDI，作品名与编号取自该表。',
            en:'Official musicnet_metadata.csv and reference MIDI.'},
